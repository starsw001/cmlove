...
if ($submitok=="addupdate") {
	...
	if($total>0){
		for($i=1;$i<=$total;$i++) {
			...
			$path1    = $rows[0];
			$path2    = $rows[1];
			if (file_exists(wrzc_net."up/photo/".$path1))unlink(wrzc_net."up/photo/".$path1);
			if (file_exists(wrzc_net."up/photo/".$path2))unlink(wrzc_net."up/photo/".$path2);
		}
	}
...
function chkform(){
if(document.wrzc_netform.userid2.value==""){
alert('请输入他/她的ID号！');
document.wrzc_netform.userid2.focus();
return false;
}
if(document.wrzc_netform.sussflag.value==""){
alert('请选择你们两个？');
document.wrzc_netform.sussflag.focus();
return false;
}
if(document.wrzc_netform.title.value==""){
alert('请输入故事主题');
document.wrzc_netform.title.focus();
return false;
}
if(document.wrzc_netform.content.value==""){
alert('请输入故事内容');
document.wrzc_netform.content.focus();
return false;
}
if(document.wrzc_netform.content.value.length<10 || document.wrzc_netform.content.value.length>10000){
...
return false;
        ...
        <form action="g_story.php" method="post" name="wrzc_netform" onSubmit="return chkform()">
          <tr>

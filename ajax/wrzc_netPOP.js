...
document.write("#wrzc_netTips {position:absolute; z-index: 100;width:256px;height:159px;BACKGROUND: url("+u+"/images/wrzc_netTips.gif) no-repeat;display:none}");
...
document.write("<div id='wrzc_netTips'>");
...
var movewrzc_net,loop1,loop2,loop3,loop4;
var w_body = 256;var h_body = 159;var w_cursor = 0;var h_cursor = 0;var l_cursor = 0;var t_cursor = 0;
var wrzc_netfrm = ( document.compatMode.toLowerCase()=="css1compat" ) ? document.documentElement : document.body;
var wrzc_netst = document.getElementById("wrzc_netTips").style;
wrzc_netst.top = ( wrzc_netfrm.clientHeight - h_body ) + "px";
wrzc_netst.left = ( wrzc_netfrm.clientWidth - w_body ) + "px";
function movewrzc_netTips() {wrzc_netst.top = ( wrzc_netfrm.scrollTop + wrzc_netfrm.clientHeight - h_body ) + "px";wrzc_netst.left = ( wrzc_netfrm.scrollLeft + wrzc_netfrm.clientWidth - w_body ) + "px";}
function CloseIt(){
var supdes_wrzc_net = confirm("确定吗？关闭后将在24小时内不显示！");
if (supdes_wrzc_net){
SetCookie("ifPOP","CLOSE");
clearInterval(loop1);clearInterval(loop2);clearInterval(loop3);clearInterval(loop4);clearInterval(SUPDES);clearInterval(movewrzc_net);wrzc_netst.display='none';
}}
...
break;
case 'S':
if (IFTIPS==1){loadSUPDES();} else {clearInterval(movewrzc_net);wrzc_netst.display='none';}
break;}}
...
function loadSUPDES(){
wrzc_netst.display='block';
movewrzc_net = setInterval("movewrzc_netTips();",100);

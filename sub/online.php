...
class online_wrzc_net{
	function chk () {
		global $db,$Global;
		extract($_COOKIE);
		$actiontime = strtotime("now");
		if ( !isset($cook_rnd) || !ereg("^[0-9]{1,9}$",$cook_rnd) || empty($cook_rnd) ){
			setcookie("cook_rnd",0,null,"/",$Global['m_cookdomain']);
			$cook_rnd = 0;}
		if ( !isset($cook_userid) || empty($cook_userid) || !ereg("^[0-9]{1,9}$",$cook_userid) ) {
	...
	function num ($str) {
		global $db,$Global;
		$Tmpsql = ( ereg("^[1-2]{1}$",$str) && !empty($str))?" WHERE sex=$str":"";

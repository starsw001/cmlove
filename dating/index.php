...
<div class="title">
	...
	<script language="javascript">
		function chk(){
			if(document.wrzc_netform.k.value==""){
			alert('请输入约会名称!');
			document.wrzc_netform.k.focus();
			return false;
			}
		}
	</script>
	<form action="./" method="get" name=wrzc_netform onsubmit="return chk()"><input name="k" type="text" class="Sinput" maxlength="20"><input type="image" src="../images/sox.gif" align=absmiddle /><input type="hidden" name="t" value="6" /></form>
	</div>
</div>
...
<!--loop start -->
		...
		switch ($t){ 
			...
			case 7:
			$tmpsubsql = '';
			$tmpsubsql .= ereg("^[1-2]{1}$",$Ssex)?" b.sex=$Ssex AND ":'';
			if (ereg("^[1-3]{1}$",$Syhtime)){
			if ($Syhtime == 1){
			...
			$tmpsubsql .= ereg("^[1-5]{1}$",$Skind)?" a.kind=$Skind AND ":'';
			if (!empty($province) && !empty($city))$tmpsubsql .= " a.area1='$province' AND a.area2='$city' AND ";
			$tmpsubsql .= ereg("^[1-4]{1}$",$Sprice)?" a.price=$Sprice AND ":'';
			$tmpsubsql .= ereg("^[1-3]{1}$",$Smaidian)?" a.maidian=$Smaidian AND ":'';

function wrzc_net_getbyid(id){
   itm = null;
   if (document.getElementById){itm = document.getElementById(id);}
   else if (document.all){itm = document.all[id];}else if (document.layers){itm = document.layers[id];}
   return itm;}
function wrzc_netAd(element,url,width,height,images,links,texts){
	if (!wrzc_net_getbyid(element)) return;
	...
	str += '</object>';
	wrzc_net_getbyid(element).innerHTML = str;
}
var focus_width=424;
...
var texts = '';
var focus_ly = '/images/wrzc_net.swf';
...
<div class="main">
	<div class="left">
		...
		if ($t == 1 && !empty($k)){
			...
			if ( ereg("^[1-5]{1}$",$kind) && !empty($kind))$tempsql       .= " kind='$kind' AND ";
			...
			if ( ereg("^[0-9]{1,3}$",$heigh1) && !empty($heigh1))$tempsql .= " heigh>='$heigh1' AND ";
			if ( ereg("^[0-9]{1,3}$",$heigh2) && !empty($heigh2))$tempsql .= " heigh<='$heigh2' AND ";
			if ( ereg("^[0-9]{1,3}$",$weigh1) && !empty($weigh1))$tempsql .= " weigh>='$weigh1' AND ";
			if ( ereg("^[0-9]{1,3}$",$weigh2) && !empty($weigh2))$tempsql .= " weigh<='$weigh2' AND ";
			if ( ereg("^[1-6]{1}$",$edu) && !empty($edu))$tempsql         .= " edu='$edu' AND ";
			if ( ereg("^[1-7]{1}$",$love) && !empty($love))$tempsql       .= " love='$love' AND ";
			if ( ereg("^[0-9]{1,2}$",$job) && !empty($job))$tempsql       .= " job='$job' AND ";
			if ( ereg("^[0-9]{1,2}$",$field) && !empty($field))$tempsql   .= " field='$field' AND ";
			if ( ereg("^[0-9]{1,2}$",$pay) && !empty($pay))$tempsql       .= " pay='$pay' AND ";
			if ( ereg("^[1-6]{1}$",$house) && !empty($house))$tempsql     .= " house='$house' AND ";
			if ( ereg("^[1-5]{1}$",$car) && !empty($car))$tempsql         .= " car='$car' AND ";
			if ( ereg("^[1-6]{1}$",$smoking) && !empty($smoking))$tempsql .= " smoking='$smoking' AND ";
			if ( ereg("^[1-6]{1}$",$drink) && !empty($drink))$tempsql     .= " drink='$drink' AND ";
			switch ($s){ 
		...
		if ($total > 0){
			...
			require_once wrzc_net.'sub/classcool.php';
			$pagesize = 14;
			if ($p<1)$p=1;
